...
# Menu items
menu_items = ["Radio", "Morse Code", "Torch", "GPS", "Compass","About"]
selected_index = 0
...
while True:
    ...
    if switch_enc.value is False:
        if selected_index == 0:
            supervisor.set_next_code_file('radioapp.py')
        if selected_index == 1:
            supervisor.set_next_code_file('MorseFlash.py')
        if selected_index == 2:
            supervisor.set_next_code_file('Torch.py')
        if selected_index == 3:
            supervisor.set_next_code_file('gps_displayio.py')
        if selected_index == 4:
            supervisor.set_next_code_file('lsm303_displayio.py')
        if selected_index == 5:
            supervisor.set_next_code_file('about.py')
        time.sleep(1)
        supervisor.reload()