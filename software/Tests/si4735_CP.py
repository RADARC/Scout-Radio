...
#      
#     def raw(self):
#         self.raw_data = bytearray([self.func | (self.xoscen << 4) | (self.patch << 5) | (self.gpo2oen << 6) | (self.ctsien << 7), self.opmode])
...
class Si47xSsbMode:
    def __init__(self):
            ...
            #self.raw_data = bytearray(2)  # Represents the raw data as a byte array
...
#                     ((self.avc_divider & 0x0F) << 8) | \
#                     ((self.avc_enabled & 0x01) << 12) | \
#                     ((self.soft_mute_selection & 0x01) << 13) | \
#                     ((self.dummy1 & 0x01) << 14) | \
#                     ((self.dsp_afc_disabled & 0x01) << 15),0]

        return [
                (self.audio_bandwidth & 0x0F) | \
                ((self.ss_cutoff_filter & 0x0F) << 4), 
                ((self.avc_divider & 0x0F)) | ((self.avc_enabled & 0x01) << 4) | \
                ((self.soft_mute_selection & 0x01) << 5) |((self.dummy1 & 0x01) << 6) | \
...
class SI4735:
    ...
    def set_device_i2c_address(self, sen_pin):
        self.device_address = SI473X_ADDR_SEN_HIGH if sen_pin else SI473X_ADDR_SEN_LOW
        
    def reset(self):
        print("Reset")
        #pin_reset = Pin(self.reset_pin, Pin.OUT,Pin.PULL_UP)
        pin_reset = DigitalInOut(self.reset_pin)
        pin_reset.direction = Direction.OUTPUT
        #pin_reset.pull = Pull.UP
        time.sleep(0.010)
    ...
    def get_device_i2c_address(self, reset_pin):
        ...
        try:
            self.i2c.writeto(SI473X_ADDR_SEN_LOW, b'0')
            self.set_device_i2c_address(0)
            self.i2c.unlock()
        ...
        try:
            self.i2c.writeto(SI473X_ADDR_SEN_HIGH, b'0')
            self.set_device_i2c_address(1)
            self.i2c.unlock()
    ...
    def waitToSend(self):
        ...
        while True:
            ...
            #out = self.i2c.readfrom(self.device_address,1)
            out=bytearray(1)
            while not self.i2c.try_lock():
                pass
            self.i2c.readfrom_into(self.device_address,out)
            self.i2c.unlock()
    ...
    def sendProperty(self,propertyNumber,parameter):
        ...
        while not self.i2c.try_lock():
            pass
        self.i2c.writeto(self.device_address,bytearray((SET_PROPERTY,0,Property.raw['byteHigh'],Property.raw['byteLow'],Param.raw['byteHigh'],Param.raw['byteLow'])))
        self.i2c.unlock()
        self.waitToSend()
    ...
    def radioPowerUp(self):
        ...
        while not self.i2c.try_lock():
            pass
        print(self.i2c.writeto(self.device_address,bytearray((POWER_UP,self.powerUp.get_raw()[0],self.powerUp.get_raw()[1]))))
        self.i2c.unlock()
        self.waitToSend()
    ...
    def getFirmware(self):
         ...
         while not self.i2c.try_lock():
            pass
         print(self.i2c.writeto(self.device_address,GET_REV))
         self.i2c.unlock()
         self.waitToSend()
         firmware=bytearray(9)
         #firmware = self.i2c.readfrom(self.device_address,9)
         while not self.i2c.try_lock():
                pass
         self.i2c.readfrom_into(self.device_address,firmware)
         self.i2c.unlock()
         #print(firmware)
    ...
    def setFrequency(self,frequency):
        ...
        while not self.i2c.try_lock():
            pass
        self.i2c.writeto(self.device_address,barray)
        self.i2c.unlock()
        ...
        with open('patch.csg', mode="rt") as f:
            ...
            while line:
                if not line.startswith("#"):
                    ...
                    while not self.i2c.try_lock():
                        pass
                    self.i2c.writeto(self.device_address, bytearray(chunk))
                    self.i2c.unlock()
                line = f.readline()
                time.sleep(MIN_DELAY_WAIT_SEND_LOOP/1_000_000)
    ...
    def disableFMDebug(self):
        ...
        while not self.i2c.try_lock():
            pass
        print(self.i2c.writeto(self.device_address,barray))
        self.i2c.unlock()
    ...
    def powerDown(self):
        ...
        while not self.i2c.try_lock():
            pass
        print(self.i2c.writeto(self.device_address,POWER_DOWN))
        self.i2c.unlock()
        self.waitToSend()
        time.sleep(2500/1_000_000)
        print("Power down")
    ...
    def getCurrentReceivedSignalQuality(self,INTACK):
        ...
        while not self.i2c.try_lock():
            pass
        self.i2c.readfrom_into(self.device_address,signal)
        self.i2c.unlock()
        #signal = self.i2c.readfrom(self.device_address,sizeResponse)
    ...
    def getRDSStatus(self,INTACK,MTFIFO,STATUSONLY):
        ...
        while not self.i2c.try_lock():
            pass
        rds=bytearray(13)
        self.i2c.readfrom_into(self.device_address,rds)
        self.i2c.unlock()
        #rds = self.i2c.readfrom(self.device_address,13)
